...
int initialize_cuda_core(Timers *timers, KernelContext *k_ctx, Args *args,
                         ...
                         ULL y_offset) {
    IF_SMAX_DEBUG(ErrorHandler::log("Entering bspmv_initialize_cuda_core"));
    ...
    if (flags->is_mat_scs) {
        ...
        ULL A_height_pad = args->A->crs->height_pad;
        ULL A_width_pad = args->A->crs->width_pad;
        ...
        transfer_HtoD<VT>(A_val, args->d_A->crs->val, A_nnz*A_height_pad*A_width_pad);

        // Copy metadata from host matrix
        args->d_A->crs->n_rows = args->A->crs->n_rows;
        args->d_A->crs->n_cols = args->A->crs->n_cols;
        args->d_A->crs->nnz = args->A->crs->nnz;
        args->d_A->crs->b_height = args->A->crs->b_height;
        args->d_A->crs->b_width = args->A->crs->b_width;
        args->d_A->crs->height_pad = args->A->crs->height_pad;
        args->d_A->crs->width_pad = args->A->crs->width_pad;
    ...
    IF_SMAX_DEBUG(ErrorHandler::log("Exiting bspmv_initialize_cuda_core"));
    return 0;
};
...
int apply_cuda_core(Timers *timers, KernelContext *k_ctx, Args *args,
    ...
    IF_SMAX_DEBUG(ErrorHandler::log("Entering bspmv_apply_cuda_core"));
    ...
    if (flags->is_mat_scs) {
        ...
        throw std::runtime_error("Blocked scs not implemented yet");
    }
    else{
        naive_bcrs_spmv_cuda_launcher<IT, VT>(
            args->d_A->crs->n_rows,
            args->d_A->crs->b_height,
            args->d_A->crs->b_width,
            args->d_A->crs->height_pad,
            args->d_A->crs->width_pad,
    ...
    IF_SMAX_DEBUG(ErrorHandler::log("Exiting bspmv_apply_cuda_core"));
    return 0;
}
...
int finalize_cuda_core(Timers *timers, KernelContext *k_ctx, Args *args,
                       Flags *flags, ULL A_offset, ULL x_offset, ULL y_offset) {
    IF_SMAX_DEBUG(ErrorHandler::log("Entering bspmv_finalize_cuda_core"));
    ...
    if (flags->is_mat_scs) {
        ...
        y_size = args->A->crs->n_rows * args->A->crs->height_pad;
    }
    ...
    IF_SMAX_DEBUG(ErrorHandler::log("Exiting bspmv_finalize_cuda_core"));
    return 0;
}
