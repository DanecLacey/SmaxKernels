...
__global__ void naive_bcrs_spmv_cuda_thread_per_row(
    ...
    VT *loc_y = reinterpret_cast<VT *>(&buffer[0] +
                                       threadIdx.x * b_height * sizeof(VT));
            ...
            if constexpr (block_column_major) {
                for (int w(0); w < b_width; ++w) {
                    for (int h(0); h < b_height; ++h) {
                        loc_y[h] += val[j * height_pad * width_pad +
                                        w * height_pad + h] *
                                    x[col[j] * width_pad + w];
                    }
                }
...
__global__ void naive_bcrs_spmv_cuda_warp_per_row(
    ...
    const int m_col =
        (thread_idx / bs < columns_per_iter) ? int(thread_idx / bs) : int(-1);
    // local row and column for this thread
    const int l_row = block_column_major ? (thread_idx % b_height)
                                         : ((thread_idx % bs) / b_width);
    const int l_col = block_column_major ? ((thread_idx % bs) / b_height)
                                         : (thread_idx % b_width);
    ...
    // offset into our actual local block buffer, here, always inflate to
    // warpSize
    VT *loc_buffer =
        reinterpret_cast<VT *>(&buffer[warpSize * warp_idx * sizeof(VT)]);

    // grid strided for loop
    for (ULL row = start_row; row < n_rows;
         row += warp_block_size * ULL(gridDim.x)) {
        // here all our threads simply set shared mem to zero
        loc_buffer[thread_idx] = VT(0);
        // loop with all active threads, where each thread handles its offset
        // into the blocked matrix
        for (IT j = row_ptr[row] + m_col; m_col >= 0 && j < row_ptr[row + 1];
             j += columns_per_iter) {
            if constexpr (block_column_major) {
                loc_buffer[m_col * bs + l_col * b_height + l_row] +=
                    val[j * height_pad * width_pad + l_col * height_pad +
                        l_row] *
                    x[col[j] * width_pad + l_col];
            } else {
                // write local values out in column major format, since this is
                // easier for reduction
                loc_buffer[m_col * bs + l_col * b_height + l_row] +=
                    val[j * height_pad * width_pad + l_row * width_pad +
                        l_col] *
                    x[col[j] * width_pad + l_col];
            }
        }
        // now reduce to first b_height threads with a cyclic reduction and
        // write out
        for (ULL cur_cutoff = power_hint; cur_cutoff >= ULL(1);
             cur_cutoff /= ULL(2)) {
            const ULL offset = cur_cutoff * b_height;
            if (thread_idx < offset) {
                loc_buffer[thread_idx] += thread_idx + offset < warpSize
                                              ? loc_buffer[thread_idx + offset]
                                              : VT(0);
            }
        }
...
__global__ void naive_bcrs_spmv_cuda_warp_per_row_by_shffl(
    ...
    for (ULL row = start_row; row < n_rows;
        ...
        for (IT j = row_ptr[row] + m_col; m_col >= 0 && j < row_ptr[row + 1];
             j += columns_per_iter) {
            loc_sum +=
                val[j * height_pad * width_pad + l_col * height_pad + l_row] *
                x[col[j] * width_pad + l_col];
        }
...
void naive_bcrs_spmv_cuda_launcher(
    ...
    const ULL height_pad, const ULL width_pad, const IT *SMAX_RESTRICT col,
    const IT *SMAX_RESTRICT row_ptr, const VT *SMAX_RESTRICT val,
    const VT *SMAX_RESTRICT x, VT *SMAX_RESTRICT y, const int krn_type,
    ...
    if (krn_type == naive_thread_per_row) {
        ...
        if (b_height * b_width > warp_size) {
            throw std::runtime_error(
                "Warp group strategy requires b x h smaller warp size");
        }
        ...
        ULL power_hint =
            ULL(next_pow_2(uint(warp_size) / (2u * uint(b_height))));
        ...
        if (!block_column_major) {
            throw std::runtime_error("BCSR spmv warp shuffle only works with "
                                     "block column major ordering");
        }
        if (b_height * b_width > warp_size) {
            throw std::runtime_error(
                "Warp group strategy requires b x h smaller warp size");
...
#define INSTANTIATE_BCRS_SPMV_KERNEL(IndexType, ValueType, ColMajor)           \
    ...
    naive_bcrs_spmv_cuda_thread_per_row<IndexType, ValueType, ColMajor>(       \
        ...
        const IndexType *SMAX_RESTRICT, const IndexType *SMAX_RESTRICT,        \
        const ValueType *SMAX_RESTRICT, const ValueType *SMAX_RESTRICT,        \
        ValueType *SMAX_RESTRICT);

#define INSTANTIATE_BCRS_SPMV_KERNEL_WARP_GROUP(IndexType, ValueType,          \
                                                ColMajor)                      \
    template __global__ void                                                   \
    naive_bcrs_spmv_cuda_warp_per_row<IndexType, ValueType, ColMajor>(         \
        ...
        const IndexType *SMAX_RESTRICT, const IndexType *SMAX_RESTRICT,        \
        const ValueType *SMAX_RESTRICT, const ValueType *SMAX_RESTRICT,        \
        ValueType *SMAX_RESTRICT, const ULL);
...
#define INSTANTIATE_BCRS_SPMV_LAUNCHER(IndexType, ValueType)                   \
    template void naive_bcrs_spmv_cuda_launcher<IndexType, ValueType>(         \
        ...
        const IndexType *SMAX_RESTRICT, const IndexType *SMAX_RESTRICT,        \
        const ValueType *SMAX_RESTRICT, const ValueType *SMAX_RESTRICT,        \
        ValueType *SMAX_RESTRICT, const int, const bool);
