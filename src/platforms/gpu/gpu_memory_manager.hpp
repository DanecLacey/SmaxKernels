...
void allocate_copy_to_device(const void *host_data, void *&device_data_out,
                             size_t n_elems) {
#if SMAX_GPU_MODE
...
void transfer_DtoH(const void *d_data, void *&h_data_out, size_t n_elems) {
    ...
    // GPU_SAFE_BACKEND_CALL(Free, (const_cast<void *>(d_data))); // highly
    // discouraged to remove const but
...
void transfer_HtoD(const void *h_data, void *&d_data_out, size_t n_elems) {
    ...
    d_data = allocGPU<elem_type>(n_elems).release();
