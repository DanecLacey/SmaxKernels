...
class gpu_stream {
  ...
  public:
    ...
    gpu_stream(stream_create_flags flag = stream_create_flags::default_stream,
        ...
        CHECK_DEVICE_ERR(GPU_BACKEND(StreamCreateWithPriority)(
    ...
    ~gpu_stream() {
        if (stream_ != nullptr) {
            CHECK_DEVICE_ERR(GPU_BACKEND(StreamDestroy)(stream_));
        }
    }
...
#endif
        CHECK_DEVICE_ERR(GPU_BACKEND(StreamSynchronize)(stream_));
    }
...
//     CHECK_DEVICE_ERR(GPU_BACKEND(StreamWaitEvent)(
...
class gpu_event {
  ...
  public:
    gpu_event(event_create_flags flag = event_create_flags::default_flag) {
        CHECK_DEVICE_ERR(GPU_BACKEND(EventCreateWithFlags)(
            &event_, static_cast<unsigned int>(flag)));
    }
    ~gpu_event() {
        if (event_ != nullptr)
            CHECK_DEVICE_ERR(GPU_BACKEND(EventDestroy)(event_));
    }
    ...
    void synchronize() const {
        ...
        CHECK_DEVICE_LASTERR("Asynchronous Error in Old call");
#endif
        CHECK_DEVICE_ERR(GPU_BACKEND(EventSynchronize)(event_));
    }
    bool query() {
    ...
    void record_in_stream(
        ...
        CHECK_DEVICE_ERR(GPU_BACKEND(EventRecordWithFlags)(
    ...
    float elapsed_time_millSec(const gpu_event &eventend) const {
        ...
        float dur{0.0};
        CHECK_DEVICE_ERR(
            GPU_BACKEND(EventElapsedTime)(&dur, this->event_, eventend.event_));
        return dur;
    }
};
