...
REGISTER_TEST(perm_test_1) {
    ...
    */
    ...
    for (int level = 0; level < expected_max_level; level++) {
        for (int idx = expected_levels[level]; idx < expected_levels[level + 1];
             idx++) {
            int expected_row = expected_perm[idx];
            for (int idy = expected_levels[level];
                 idy < expected_levels[level + 1]; idy++) {
    ...
    */
    ...
    for (int row_idx = 0; row_idx < A_n_rows; row_idx++) {
        for (int idx = A_row_ptr[row_idx]; idx < A_row_ptr[row_idx + 1];
             idx++) {
            if (A_col[idx] !=
                    perm_bfs[A_bfs_col[A_bfs_row_ptr[inv_perm_bfs[row_idx]] +
                                       idx - A_row_ptr[row_idx]]] ||
                A_val[idx] != A_bfs_val[A_bfs_row_ptr[inv_perm_bfs[row_idx]] +
                                        idx - A_row_ptr[row_idx]]) {
                check_bfs = false;
            }
            /*
            if (A_col[idx] !=
            perm_dfs[A_dfs_col[A_dfs_row_ptr[inv_perm_dfs[row_idx]] + idx -
            A_row_ptr[row_idx]]] || A_val[idx] !=
            A_dfs_val[A_dfs_row_ptr[inv_perm_dfs[row_idx]] + idx -
            A_row_ptr[row_idx]] ){ check_dfs = false;
            }
            */
            if (A_col[idx] !=
                    perm_jh[A_jh_col[A_jh_row_ptr[inv_perm_jh[row_idx]] + idx -
                                     A_row_ptr[row_idx]]] ||
                A_val[idx] != A_jh_val[A_jh_row_ptr[inv_perm_jh[row_idx]] +
                                       idx - A_row_ptr[row_idx]]) {
                check_jh = false;
            }
        }
