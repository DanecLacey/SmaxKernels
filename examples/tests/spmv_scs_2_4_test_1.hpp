...
REGISTER_TEST(spmv_scs_2_4_test_1) {
    ...
    SCSMatrix<IT, VT> *A_scs = new SCSMatrix<IT, VT>(2, 4);
    ...
    SCSMatrix<IT, VT> *A_scs_exp = new SCSMatrix<IT, VT>(2, 4);
    A_scs_exp->n_rows = 3;
    A_scs_exp->n_rows_padded = 4;
    A_scs_exp->n_cols = 3;
    A_scs_exp->n_chunks = 2;
    A_scs_exp->n_elements = 6;
    A_scs_exp->nnz = 5;
    A_scs_exp->chunk_ptr = new IT[A_scs_exp->n_chunks + 1]{0, 4, 6};
    A_scs_exp->chunk_lengths = new IT[A_scs_exp->n_chunks]{2, 1};
    A_scs_exp->col = new IT[A_scs_exp->n_elements]{
        0, 0, 1, 2, 1, 0,
    };
    A_scs_exp->val = new VT[A_scs_exp->n_elements]{
        1.1, 3.1, 1.2, 3.3, 2.2, 0.0,
    };
